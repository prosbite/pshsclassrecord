...
const isEdit = computed(() => Boolean(props.assessment?.id));

const form = useForm({
    title: props.assessment?.title ?? '',
    assessment_type_id: props.assessment?.assessment_type_id ?? null,
    quarter_id: props.assessment?.quarter_id ?? null,
    section_id: props.assessment?.section_id ?? null,
    school_year_id: props.assessment?.school_year_id ?? props.schoolYear?.id ?? null,
    learner_scores: [],
...
const learnerTentatives = reactive({});
const csvStatus = ref('');
...
const resetLearnerScores = (list) => {
    ...
    Object.keys(learnerTentatives).forEach((key) => delete learnerTentatives[key]);
    ...
    list.forEach((enrollment) => {
        ...
        const initial = props.initialScores?.[learnerId] ?? props.initialScores?.[String(learnerId)];
        learnerScores[learnerId] = initial !== undefined && initial !== null ? Number(initial) : 0;

        const initialTentative = props.initialTentatives?.[learnerId]
            ?? props.initialTentatives?.[String(learnerId)];
        learnerTentatives[learnerId] = Boolean(initialTentative);
    });
};

const masterTentative = computed({
    get: () => hasLearners.value
        && selectedLearners.value.every((learner) => Boolean(learnerTentatives[learner.id])),
    set: (value) => {
        selectedLearners.value.forEach((learner) => {
            learnerTentatives[learner.id] = Boolean(value);
        });
    },
});

const masterTentativeIndeterminate = computed(() =>
    !masterTentative.value
    && selectedLearners.value.some((learner) => Boolean(learnerTentatives[learner.id])));
...
const parseTentative = (value) =>
    ['1', 'true', 'yes', 'y', 't'].includes((value ?? '').toString().trim().toLowerCase());
...
const handleScoresCsv = async (event) => {
    ...
    try {
        ...
        const hasTentativeColumn = headerCols.includes('tentative');
        ...
        lines.slice(1).forEach((line) => {
            ...
            if (hasTentativeColumn) {
                learnerTentatives[learnerId] = parseTentative(values[index('tentative')]);
            }
...
const handleSubmit = () => {
    ...
    if (isEdit.value) {
        form.put(route('assessments.update', props.assessment.id), {
            preserveScroll: true,
            onSuccess: () => {
                router.visit(route('assessments.show', props.assessment.id));
            },
        });
...
const heading = computed(() => (isEdit.value
    ? { eyebrow: 'Edit assessment', title: 'Update this evaluation', description: 'Adjust the assessment details and per-learner scores.' }
    : { eyebrow: 'Create assessment', title: 'Draft a new evaluation', description: 'Capture the title, type, quarter, and section for the assessment you are scheduling.' }));
</script>
...
<template>
    <div class="rounded-3xl bg-white p-6 shadow-lg sm:p-8">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-xs uppercase tracking-[0.45em] text-slate-400">{{ heading.eyebrow }}</p>
                <h1 class="text-2xl font-semibold text-slate-900">{{ heading.title }}</h1>
        ...
        <form class="mt-8 space-y-6" @submit.prevent="handleSubmit">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <label class="block">
                    <span class="text-xs font-semibold uppercase tracking-[0.4em] text-slate-400">Perfect score</span>
                    <input
                        v-model.number="form.perfect_score"
                        type="number"
                        min="0"
                        step="1"
                        class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-slate-400 focus:bg-white focus:outline-none"
                    />
                    <InputError :message="form.errors.perfect_score" class="mt-1" />
                </label>
            </div>
            ...
            <div class="rounded-3xl border border-slate-100 bg-slate-50 p-6 shadow-inner space-y-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <span class="text-xs text-slate-500">
                        Columns required: last_name, first_name, assessment_type, scores. Optional: tentative.
                    </span>
                        ...
                        <thead class="bg-white text-slate-500">
                            <tr>
                                ...
                                <th class="px-4 py-3 font-semibold text-center">
                                    <label v-if="hasLearners" class="inline-flex cursor-pointer items-center gap-2">
                                        <input
                                            type="checkbox"
                                            :checked="masterTentative"
                                            :indeterminate="masterTentativeIndeterminate"
                                            class="h-4 w-4 rounded border-slate-300 text-amber-500 focus:ring-amber-400"
                                            @change="masterTentative = $event.target.checked"
                                        />
                                        <span>Tentative</span>
                                    </label>
                                    <span v-else>Tentative</span>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            >
                                ...
                                <td class="px-4 py-3 text-center">
                                    <input
                                        v-model="learnerTentatives[learner.id]"
                                        type="checkbox"
                                        class="h-4 w-4 rounded border-slate-300 text-amber-500 focus:ring-amber-400"
                                        :aria-label="`Mark ${learner.name} as tentative`"
                                    />
                                </td>
                            </tr>
                        </tbody>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                    ...
                    <p class="text-xs text-slate-500">Saved on behalf of the signed-in teacher.</p>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 rounded-3xl border border-slate-200 bg-slate-50 p-4">
                ...
                <PrimaryButton type="submit" :disabled="!canSubmit">
                    {{ form.processing ? 'Saving…' : (isEdit ? 'Update assessment' : 'Save assessment') }}
                </PrimaryButton>
            </div>
        </form>
    </div>
